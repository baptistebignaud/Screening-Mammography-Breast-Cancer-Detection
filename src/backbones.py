...
class ResNet(nn.Module):
    def __init__(self, input_channels, block, blockList, featureMaps, large = False):
        super(ResNet, self).__init__()
        self.large = large
        if not large:
            self.embed = ConvBN2d(input_channels, featureMaps, outRelu = True)
        else:
            self.embed = ConvBN2d(input_channels, featureMaps, kernel_size=7, stride=2, padding=3, outRelu = True)
...
class ResNet12(nn.Module):
    def __init__(self, input_channels, featureMaps):
        super(ResNet12, self).__init__()
        self.block1 = BasicBlockRN12(input_channels, featureMaps)
...
def prepareBackbone():
    ...
    return {
        "resnet18": lambda: (ResNet(args.input_channels, BasicBlock, [(2, 1, 1), (2, 2, 2), (2, 2, 4), (2, 2, 8)], args.feature_maps, large = large), 8 * args.feature_maps),
        "resnet20": lambda: (ResNet(args.input_channels, BasicBlock, [(3, 1, 1), (3, 2, 2), (3, 2, 4)], args.feature_maps, large = large), 4 * args.feature_maps),
        "resnet56": lambda: (ResNet(args.input_channels, BasicBlock, [(9, 1, 1), (9, 2, 2), (9, 2, 4)], args.feature_maps, large = large), 4 * args.feature_maps),
        "resnet56flat": lambda: (ResNet(args.input_channels, BasicBlock, [(9, 1, 1), (9, 2, 1.41), (9, 2, 2)], args.feature_maps, large = large), 2 * args.feature_maps),
        "resnet110": lambda: (ResNet(args.input_channels, BasicBlock, [(18, 1, 1), (18, 2, 2), (18, 2, 4)], args.feature_maps, large = large), 4 * args.feature_maps),
        "resnet50": lambda: (ResNet(args.input_channels, BottleneckBlock, [(3, 1, 1), (4, 2, 2), (6, 2, 4), (3, 2, 8)], args.feature_maps, large = large), 8 * 4 * args.feature_maps),
        "resnet12": lambda: (ResNet12(args.input_channels, args.feature_maps), 10 * args.feature_maps),
        "wrn28_10": lambda: (ResNet(args.input_channels, BasicBlock, [(4, 1, 10), (4, 2, 20), (4, 2, 40)], args.feature_maps, large = large), 40 * args.feature_maps),
        "wrn16_16": lambda: (ResNet(args.input_channels, BasicBlock, [(2, 1, 16), (2, 2, 32), (2, 2, 64)], args.feature_maps, large = large), 64 * args.feature_maps),
